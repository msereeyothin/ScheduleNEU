...
function WeekDisplay({ section }: { section: Section }) {
  ...
  const uniqueTimes = Array.from(new Set(dayTimePairs.map(pair => pair.timeString)));

  return (
    <div>
      {section.meetings.map((meeting, meetingIndex) => (
        <Box key={meetingIndex} sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Box sx={{ display: "flex", flexDirection: "row" }}>
            {Array.from({ length: 5 }, (_, i) => i + 1).map((dayNum) => {
              const activated = Object.keys(meeting.times).map(day => parseInt(day)).includes(dayNum);
          ...
          {uniqueTimes.map((time, index) => {
            const isTimeForDay = dayTimePairs.some(pair => pair.timeString === time && Object.keys(meeting.times).map(day => parseInt(day)).includes(pair.day));
